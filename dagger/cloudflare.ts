...
const wranglerImage = 'michaelmass/wrangler:0.0.6'
...
export async function upload({ client, dir = '.', project, cloudflareToken, accountId, branch, commitHash, commitMessage, commitDirty }: UploadOptions) {
  const directory = typeof dir === 'string' ? client.host().directory(dir) : dir

  const exec = formatUploadCommad({ project, branch, commitHash, commitMessage, commitDirty })
...
export function container({ client, dir = '.', cloudflareToken, accountId }: ContainerOptions) {
  const directory = typeof dir === 'string' ? client.host().directory(dir) : dir
...
type FormatUploadCommadOptions = {
  /**
   * The name of the project you want to deploy to
   */
  project: string
...
export async function whoami({ client, cloudflareToken, accountId }: WhoAmIOptions) {
  await client
    .container()
    .from(wranglerImage)
    .withSecretVariable('CLOUDFLARE_ACCOUNT_ID', accountId)
