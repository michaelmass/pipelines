...
export async function upload({ client, dir = '.', bucket, prefix = '', awsRegion = 'us-east-1', cloudfrontDistribution, awsAccessKeyId, awsSecretAccessKey }: UploadOptions) {
  ...
  let container = client
    ...
    .withWorkdir('/src')
    .withExec(['aws', 's3', 'sync', '.', `s3://${bucket}/${prefix}`, '--delete'])

  if (cloudfrontDistribution) {
    container = container.withExec(['aws', 'cloudfront', 'create-invalidation', '--distribution-id', cloudfrontDistribution, '--paths', prefix ? `/${prefix}/*` : '/*'])
  }
