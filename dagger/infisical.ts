...
export function getInfinsical({
  ...
  return {
    list: async ({ secretPath, prefix = '' }: { secretPath?: string; prefix?: string } = {}) => {
      ...
      return Object.fromEntries(Object.entries(secrets).map(([key, value]) => [key, client.setSecret(`${prefix}${key}`, value)]))
    },
    get: async ({ name, secretName, secretPath }: { name: string; secretName: string; secretPath?: string }) => {
...
export async function getInfisicalSecrets({ environment, workspaceId, token, secretPath }: GetInfisicalSecretsOptions) {
  const response = await fetch(
    `https://app.infisical.com/api/v3/secrets/raw?environment=${environment}&workspaceId=${workspaceId}&secretPath=${encodeURIComponent(secretPath.startsWith('/') ? secretPath : `/${secretPath}`)}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  return (data.secrets?.reduce(
    (acc: Record<string, string>, secret: { secretKey: string; secretValue: string }) => {
