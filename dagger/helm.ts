...
export async function deploy({
  ...
  const kubeconfigFile = typeof kubeconfig === 'string' ? client.directory().withNewFile(kubeconfigPath, kubeconfig).file(kubeconfigPath) : kubeconfig
  ...
  const valuesFile =
    values instanceof File
      ...
      : client
          .directory()
          .withNewFile(valuesPath, values === 'string' ? values : stringify(values))
          .file(valuesPath)
  ...
  await client.container().from('alpine/helm:3.16.4').withFile(kubeconfigPath, kubeconfigFile).withFile(valuesPath, valuesFile).withExec(exec).sync()
}
